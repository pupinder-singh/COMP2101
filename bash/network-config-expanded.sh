...
declare -a inter=($interface)
# A loop generate for processing each work in the array
for i in "${inter[@]}"; do
# For bypass loopback interface, use conditions
    if [[ $i = lo* ]] ;
		then continue ;
	fi
 #For assigning command's output to the variable, use simple lines
